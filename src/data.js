...
export const projects = [  {
  ...
  },
  {
    ...
    highlights: [
      ...
      "Bought a real corpus a bug the clean one could never show: on 100 photographed Philippine receipts the strongest check was silent 79 times out of 100, because the tills print a subtotal and the camera reads it as “Sabtotal” — every label in the parser was an exact regex, so one wrong letter switched the guard off. Matching by likeness instead (0.8, because 0.7 read TOTAL as SUBTOTAL) took subtotals found from 21 to 35 and passing item-sum checks from 6 to 22",
      "Fixing that uncovered an older bug it had been hiding: the basket parser stopped at the totals block by exact regex too, so “IOTAL. DUE” and “Vetable Sales” were counted as things somebody bought — one receipt’s six items included the total, the cash tendered and the VAT-exempt line. It had never been noticed because the check that would have caught it was skipping for want of a subtotal",
